...
matching. Nothing on your machine is wrong. Check it is really that (the
tarball should unpack to the same tree as the upstream git tag), then build
with the checksum step skipped:

```bash
yay -S wallust --mflags --skipchecksums
```

If the AUR maintainer has already refreshed the checksum, plain `yay -S wallust`
works again and the flag is unnecessary.
