...
DropdownWidget {
    ...
    Process {
        id: profileMonitor
        command: ["sh", "-c", "dbus-monitor --system \"type='signal',interface='org.freedesktop.DBus.Properties',member='PropertiesChanged',path='/net/hadess/PowerProfiles'\" 2>/dev/null | while read line; do echo 'profile_changed'; done"]
        running: true
        stdout: SplitParser {
            onRead: data => {
                if (data) {
