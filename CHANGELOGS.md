...
- Review pass (2026-09-13, four reviewers over the whole repo), fixed:
  - `limine.sh`: the enroll-failure rollback restored the first-ever backup (stale kernel hashes on a re-run) - now restores this run's pre-image; refuses to edit when enrollment is on but `limine-enroll-config` is missing; CRLF configs survive
  - `bios-logo/mod-bios-logo.sh`: the diff-range and header checks are now fatal and bounded to the logo's volume; `usb` resolves symlinks and requires `TYPE=disk`, refuses disks carrying system mount points or LUKS/LVM/RAID; free-space budget bounded to the volume; UEFIReplace exit status checked; unrecognised JPEG reports instead of exiting silently; the stick is unmounted on failure
  - `bluetooth.sh`: the sudoers drop-in is built in a temp file and validated with visudo (an empty `$USER` used to disable all sudo)
  - `02-Final-Check.sh`: quickshell and xdph checked only when selected (false fail); the nopasswd check looks for the rule instead of `sudo -n true`, which the installer's keepalive made always pass; limine check uses the same five paths as the installer
  - `install.sh`: cds to its own directory (relative `--preset` resolved first); sudo is authenticated before the first sudo call (Limine detection); explicit `bluetooth="ON"`/`rog="ON"` are no longer downgraded when detection misses; plymouth detection reads `/etc/mkinitcpio.conf.d/*.conf` too
  - swallowed exit statuses (`cmd | tee` tests tee): `pacman.sh -Syu`, `paru.sh -Syu`, `nvidia.sh` initramfs (now `limine-mkinitcpio` where present), `ly_config.sh` copies (now `install`, fatal), `01-hypr-pkgs.sh`/`ly.sh` PIPESTATUS, `locales.sh` locale-gen; `graphics.sh`/`ucode.sh` no longer die under `set -e` on an empty grep; `zsh.sh` tests for `oh-my-zsh.sh`, not the directory the plugin clones create
  - `services.sh`: `mdns_minimal` inserted before `resolve` (after it, it was never consulted); resolved's own mDNS responder disabled when avahi is installed
  - dotfiles: `WallustSwww.sh` reads the wallpaper path from `awww query` (the cache dir moved and the awk field cut names with spaces - wallust never refreshed after a wallpaper change); `initial-boot.sh` no longer cycles the keyboard layout at first login (with three layouts it landed on Spanish); `DarkLight.sh` no longer blanks the Qt colour scheme; `awww-daemon` is started in the background in the three scripts that could block on it; `copy.sh` backs up shell rc files and keeps the first-boot and wallpaper markers across a re-run; `vpn-sync.sh`/`vpn-reset.sh` use `sudo -n`, stop before writing caches when sudo refuses, and restore the machine's own saved timezone instead of Madrid; the lock scripts fall back to all monitors when the geometry generator fails
  - `make-wallpaper.sh` shebang, message and non-16:9 sizing; `verify-before-transfer.sh` runs from the repo
- `handy.sh` no longer writes its own copy of `UserScripts/handy-start.sh`, a second CTRL+SUPER+F8 bind and a second autostart handler over the dotfiles' versions. The copies had drifted (it checked `settings.json`, Handy writes `settings_store.json`), the copy lacked the missing-binary guard and first-login hint, and the autostart it appended baked an absolute `$HOME` path into a portable config. The script installs the packages and reports whether the dotfiles' launcher and bind are present
