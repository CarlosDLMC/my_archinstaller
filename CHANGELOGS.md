...
- `assets/limine/`: Limine boot-menu theme - KGB data-centre wallpaper with the Cyrillic title painted in (Limine's font has no Cyrillic), green CRT palette, `theme.conf` global block, `make-wallpaper.sh` to regenerate from a Real-ESRGAN 4x upscale of the source; the two commands to apply it by hand. The live ISO's pretty menu was GRUB, the installed Limine ships bare
